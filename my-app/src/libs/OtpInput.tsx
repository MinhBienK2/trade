...
class SingleOtpInput extends PureComponent<any> {
  ...
  componentDidMount() {
    const { focus, shouldAutoFocus } = this.props as any;
  ...
  getType = () => {
    const { isInputSecure, isInputNum } = this.props as any;
...
class OtpInput extends Component {
  ...
  getPlaceholderValue = () => {
    const { placeholder, numInputs } = this.props as any;
  ...
  isInputValueValid = value => {
    const isTypeValid = (this.props as any).isInputNum ? !isNaN(parseInt(value, 10)) : typeof value === 'string';
  ...
  focusInput = input => {
    const { numInputs } = this.props as any;
  ...
  handleOnPaste = e => {
    ...
    const { activeInput } = this.state;
    const { numInputs, isDisabled } = this.props as any;
  ...
  handleOnInput = e => {
    if (this.isInputValueValid(e.target.value)) {
      ...
      if (!(this.props as any).isInputNum) {
        const { nativeEvent } = e;
  ...
  render() {
    const { containerStyle } = this.props as any;
